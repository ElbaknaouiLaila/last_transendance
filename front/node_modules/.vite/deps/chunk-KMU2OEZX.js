...
function useList(params) {
  ...
  const handleHighlightChange = React6.useCallback((event, value, reason) => {
  ...
  const stateComparers = React6.useMemo(() => ({
    highlightedValue: itemComparer,
    selectedValues: (valuesArray1, valuesArray2) => areArraysEqual(valuesArray1, valuesArray2, itemComparer)
  }), [itemComparer]);
  const handleStateChange = React6.useCallback((event, field, value, reason, state2) => {
  ...
  const listActionContext = React6.useMemo(() => {
    return {
      disabledItemsFocusable,
      disableListWrap,
      focusManagement,
  ...
  const actionContext = React6.useMemo(() => _extends({}, reducerActionContext, listActionContext), [reducerActionContext, listActionContext]);
  ...
  const previousItems = React6.useRef([]);
  React6.useEffect(() => {
  ...
  const getItemState = React6.useCallback((item) => {
...
function useForcedRerendering() {
  const [, setState] = React8.useState({});
...
function useListItem(parameters) {
  ...
  const createHandleClick = React9.useCallback((externalHandlers) => (event) => {
  ...
  const createHandlePointerOver = React9.useCallback((externalHandlers) => (event) => {
...
//# sourceMappingURL=chunk-KMU2OEZX.js.map
